...
const DoctorComplain = () => {
  ...
  useEffect(() => {
    ...
    if (type == "pending") {
      dispatch(getDoctorPendingComplain(payload));
    } else {
      dispatch(getDoctorSolvedComplain(payload));
    }
  ...
  const handleDelete = (id: any) => {
    ...
    data
      .then((logouts) => {
        ...
        if (yes && type == "pending") {
          dispatch(deleteDoctorPendingComplain(id));
        } else {
          dispatch(deleteDoctorSolveComplain(id));
        }
      })
      ...
      Cell: ({ row }) => (
        <span>
          <button
            ...
            onClick={() =>
              dispatch(openDialog({ type: "doctorpendingcomplain", data: row }))
            }
          >
            <svg
              width="24"
